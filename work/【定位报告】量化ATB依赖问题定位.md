# 量化链路 ATB 依赖与旧 QuantFA 接口问题定位报告
...
| 项目 | 配置 |
|---|---|
| 量化路径 | MindIE-SD W8A8 静态量化、旧 QuantFA |
| 依赖对照 | 仅安装 torch_npu；安装 CANN NNAL/torch_atb |
| 复现入口 | 导入量化模块、构造量化 Linear、执行推理及部署安装 |
| 判定条件 | 无 torch_atb 环境下常规 W8A8 是否可初始化并执行 |
